...
import { Strategy } from "./DashboardContent";



const CenterPanel = (strategies: Strategy[] | any) => {
  const { selectedStrategy, setSelectedStrategy }: any = useStrategies();
  const exists = strategies && strategies.length > 0
  const data = exists ? strategies : Array.from({ length: 3 }, (_, index) => ``);
